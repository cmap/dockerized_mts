...
LFC_TABLE %<>%
  ...
  dplyr::mutate(LFC = logMFI_norm - median(logMFI_norm[pert_type == "ctl_vehicle" & pert_iname == pert_vehicle])) %>%
  dplyr::ungroup() %>%
...
LFC_TABLE %<>%
  ...
  tidyr::unite(col = "condition", pert_iname, pert_dose, compound_plate, pert_time, pert_vehicle, any_of("x_project_id"),
               sep = "::", remove = FALSE) %>%
  split(.$condition) %>%
