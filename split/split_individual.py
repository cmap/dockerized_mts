...
def build_parser():
    ...
    parser.add_argument('--pert', '-p', help='Pert name', required=True)
    parser.add_argument('--pert_plate', '-pp', help='Pert name', required=True)
...
def main(args):
    ...
    project_outdir = os.path.join(out, project,pert_plate)
    if not os.path.exists(project_outdir):
        os.makedirs(project_outdir)

    project_data = level4[
        level4['pert_iname']== pert  &
