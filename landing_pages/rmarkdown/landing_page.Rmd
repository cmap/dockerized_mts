...
for (i in 1:length(plates)) {
  ...
  report_list[[plate]] <-  paste0("./", plate, list.files(plates[[i]], pattern = "_report.html", recursive = T))
}
names(report_list <- )
if (length(report_files) > 0) {
  report_files <- paste0("./", report_files)
  names(report_files) <- stringr::word(report_files, 2, sep = fixed("/")) %>% toupper(.)
}
drc_files <- list.files(params$data_dir, pattern = "_drc.html", recursive = T)
if (length(report_files) > 0) {
  drc_files <- paste0("./", drc_files)
  names(drc_files) <- stringr::word(drc_files, 2, sep = fixed("/")) %>% toupper(.)
}

# get paths to data files
data_files <- list.files(file.path(params$data_dir, "data"))
data_files <- paste0("./", data_files)
names(data_files) <- stringr::word(data_files, 4, -2, sep = fixed("_"))
file_text <- c(
  "QC" = "QC metrics",
...
if (length(report_files) > 0) {
  for (i in 1:length(report_files)) {
    cat("* [", names(report_files)[i], "](", report_files[i], ") \n")
  }
} else {
...
if (length(drc_files) > 0) {
  for (i in 1:length(drc_files)) {
    cat("* [", names(drc_files)[i], "](", drc_files[i], ") \n")
  }
} else {
  cat("No dose-response curve reports found for this project.")
}
