...
lfc_path <- list.files(compound_dir, "LEVEL4_LFC_COMBAT", full.names = T)
...
# dose response
drc <- data.table::fread(file.path(params$data_dir, params$comp, "DRC_TABLE.csv"))
...
p <- plot_ly() %>% 
  add_trace(data = curves_all,
            x = ~x, y = ~y, type = "scatter", mode = "lines", line = list(shape = "line"),
            hoverinfo = "text", text = ~ccle_name, opacity = 0.1) %>%
  add_trace(data = lfc_all,
            x = ~Dose, y = ~Viability, type = "scatter", mode = "markers",
            hoverinfo = "text", text = ~Viability, opacity = 0.1, color = ~Replicate) %>%
...
dt <- DT::datatable(compounds_all, style="bootstrap",
                    ...
                    colnames = c("Compound", "Cell Line", "Culture", "Time", "a", "b", "s", "EC50", "AUC", "IC50", "ID"),
                    rownames = FALSE,
