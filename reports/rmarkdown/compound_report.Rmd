---
params:
  data_dir: ~/Desktop/test_reports/PMTS046
  comp: BRD-K05804044
...
# file paths
compound_dir <- file.path(params$data_dir, params$comp)
lfc_path <- list.files(compound_dir, "LEVEL5_LFC_COMBAT", full.names = T)
drc_path <- list.files(compound_dir, "DRC_TABLE", full.names = T)
...
corr_table <- data.table::fread(file.path(compound_dir, "continuous_associations.csv")) %>%
  dplyr::mutate(feature = word(feature, 2, -1, sep = "_"))
disc_table <- data.table::fread(file.path(compound_dir, "discrete_associations.csv")) %>%
  dplyr::mutate(feature = word(feature, 2, -1, sep = "_"))
model_table <- data.table::fread(file.path(compound_dir, "model_table.csv"))
rf_table <- data.table::fread(file.path(compound_dir, "RF_table.csv")) %>%
  dplyr::left_join(model_table)
if (any(corr_table$pert_time %>% unique() == "") && corr_table$pert_time %>% unique() %>% length <= 2) {
  corr_table$pert_time <- "120H"
  disc_table$pert_time <- "120H"
  model_table$pert_time <- "120H"
  rf_table$pert_time <- "120H"
}
