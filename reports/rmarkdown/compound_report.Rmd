---
params:
  data_dir: "/Users/shiker/Documents/reports/BRD-K00005265"
  comp: "BRD-K00005265"
...
if (!is.null(DRC)) {
  ...
  top_muts_names <- top_muts$feature[1:20]
  df$gene = word(df$mutation, 2, sep = '_')
  top_muts_genes <- word(top_muts_names, 2, sep = '_')
  
  df %<>%
    dplyr::filter(gene %in% top_muts_genes)
  

  p <- df %>%
    ggplot(aes(y = auc, x = reorder(mutation, auc, function(x) -mean(x, na.rm = T)),
               color = mutation)) +
    geom_boxplot(outlier.alpha = 0) +
    geom_hline(yintercept = m, linetype = "dashed") +
    geom_jitter() +
