---
params:
  data_dir: "/Users/shiker/Documents/archive/2024-Q2_report_test/PMTS056/BRD-U00116474"
  #data_dir: "/Users/shiker/Documents/trouble_shoot/ULA/MRTX-1133_shiker"
  comp: "/Users/shiker/Documents/archive/2024-Q2_report_test/PMTS056/BRD-U00116474"
  #comp: "MRTX-1133"
  is_combination: FALSE
  meta_folder: "https://s3.amazonaws.com/biomarker.clue.io"
  lfc_five_pattern: "LEVEL5_LFC"
output:
  html_document:
    toc: false
    theme: paper
...
feature_full <- c("GE" = "Gene expression",
                  ...
                  "miRNA" = "Micro RNA",
                  "PROT_BROAD" = "Proteomics (Broad)",
                  "CNA" = "Copy number",
                  "MET" = "Metabolomics",
                  "shRNA" = "shRNA knockdown",
                  "REP" = "Repurposing compounds",
                  "GSE" = "Gene Set Enrichment", # ADDING NEW FEATURE SET 
                  "PROT_GDSC" = "Proteomics (Sanger)", # ADDING NEW FEATURE SET 
                  "RPPA" = "Proteomics (RPPA)") # ADDING NEW FEATURE SET 
...
lineages <- data.table::fread(file.path(params$meta_folder, "annotations", "lineages.csv")) %>%
  dplyr::inner_join(LFC %>% dplyr::distinct(ccle_name), by = "ccle_name")
mutations <- data.table::fread(file.path(params$meta_folder, "annotations", "mutations.csv")) %>%
  dplyr::inner_join(LFC %>% dplyr::distinct(ccle_name), by = "ccle_name")
...
write.csv(compounds, file.path(exp_details_path, 'experimental_details.csv'), row.names=FALSE)
```
...
write.csv(viability_table_dt, file.path(viability_path, 'viability.csv'), row.names=FALSE)
...
# and type of gene mutation (dam, hs, other, wildtype) 
  ...
  top.genes = top_muts %>% 
    ...
    if (nrow(top.genes) == 0) {
    plot_data <- NULL 
    write.csv(plot_data, file.path(mutations_path, 'mutation_effect_boxplot_data.csv'), row.names=FALSE)
...
write.csv(feat_plot_df, file.path(correlation_analysis_path, 'correlation_analysis_data_table.csv'), row.names=FALSE)
```
...
write.csv(complete_model_data, file.path(mv_biomarker_path, 'multivariate_biomarker_complete_volcano.csv'), row.names=FALSE)
write.csv(complete_model_dt, file.path(mv_biomarker_path, 'multivariate_biomarker_complete_data_table.csv'), row.names=FALSE)
```
