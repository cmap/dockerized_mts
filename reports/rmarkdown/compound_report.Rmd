...
params:
  ...
  meta_folder: "https://s3.amazonaws.com/biomarker.clue.io/annotations/"
...
meta_folder <- gsub("/$", "", params$meta_folder)
lineages <- data.table::fread(file.path(meta_folder, "lineages.csv")) %>%
  dplyr::inner_join(LFC %>% dplyr::distinct(ccle_name), by = "ccle_name")
mutations <- data.table::fread(file.path(meta_folder, "mutations.csv")) %>%
  dplyr::inner_join(LFC %>% dplyr::distinct(ccle_name), by = "ccle_name")
...
This compound was screened in the PRISM assay at `r n_doses`-point dose (3-fold dilution) with a `r n_days` day treatment with `r n_lines` cancer cell lines passing QC. Two PRISM cell line collections were used in the assay: PR500 (including only adherent cell lines), and PR300+ (including adherent and suspension cell lines). Three benchmark compounds – included in the Validation set – were also tested at dose to ensure high data quality. All compounds were run in triplicate, and each plate contained positive (`r pos_control`) and negative (`r neg_control`) controls.
...
# and type of gene mutation (dam, hs, other, wildtype) 
  ...
  top_muts <- mutations %>% 
    ...
    dplyr::group_by(gene, Var1) %>%
    dplyr::mutate(flag = all(value == 0)) %>%  
    dplyr::ungroup() %>% 
    dplyr::mutate(mut.type = ifelse(flag, "wild_type",  mut.type)) %>%  # cell lines that doesn't have particular mutation
    dplyr::select(-flag) %>% 
    dplyr::distinct() %>%
