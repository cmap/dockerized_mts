---
params:
  data_dir: "/Users/shiker/Documents/trouble_shoot/aps001/aps001_validation_compounds/APS001_VALIDATION_COMPOUNDS/PAPS001/BRD-K51313569"
  comp: "BRD-K51313569"
...
rf_table <- data.table::fread(file.path(params$data_dir, "RF_table.csv")) %>%
  dplyr::left_join(model_table, by = c("model", "pert_iname", "pert_id", "pert_time", "pert_plate", "pert_dose"))
...
lin_tab_dt = lin_tab %>%
                ...
                dplyr::distinct(lineage, lin_abbreviation, n_lines,
                                effect_size, q.value, pert_iname, pert_dose, neg_log10_qval) %>%
...
# and type of gene mutation (dam, hs, other, wildtype) 
  ...
  top_muts <- MUT_table %>% 
    dplyr::filter(pert_dose == "log2.auc", feature %in% muts) %>% # picking dose of log2.auc and mutations within muts 
  ...
  ## for debugging #####
  print ("entering topgenes joins")
  print(length(top.genes$gene))
  write.csv(top.genes, file.path(params$data_dir, 'debugging_top_genes.csv'), row.names=FALSE)
  write.csv(mutations, file.path(params$data_dir, 'debugging_mutations.csv'), row.names=FALSE)
  write.csv(DRC, file.path(params$data_dir, 'debugging_DRC.csv'), row.names=FALSE)
  
  
  mutated_mutations <- mutations %>% 
    ...
    dplyr::mutate(ix = 1)
  write.csv(mutated_mutations, file.path(params$data_dir, 'debugging_mutated_mutations.csv'), row.names=FALSE)
 
  pre_join_table <- mutations %>% 
    dplyr::distinct(mutation)  %>% # determining unique mutations 
    dplyr::mutate(gene = word(mutation, 2, sep = fixed("_")), # adding gene word
                  mut.type = word(mutation,1,-2, sep = fixed("_"))) %>% # adding mutation type 
    dplyr::filter(gene %in% top.genes$gene) %>% # filtering from all the genes to only top genes 
    ...
    dplyr::rename(ccle_name = Var1) %>% dplyr::select(-value, -Var2) 
  
  write.csv(pre_join_table, file.path(params$data_dir, 'debugging_pre_join_table.csv'), row.names=FALSE)
  #####
  ...
  ## for debugging #####
  print ("done with top genes joins")
  #####
  plot_data <- top_muts %>%
