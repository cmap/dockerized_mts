---
params:
  data_dir: ~/Desktop/full_folder_test/MTS018_VALIDATION_COMPOUNDS/PMTS046/BRD-A12230535
  comp: BRD-A12230535
...
corr_table %<>% dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                                                 signif(as.numeric(pert_dose), 5),
                                                 pert_dose))
disc_table %<>% dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                                                 signif(as.numeric(pert_dose), 5),
                                                 pert_dose))
model_table %<>% dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                                                 signif(as.numeric(pert_dose), 5),
                                                 pert_dose))
rf_table %<>% dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                                                 signif(as.numeric(pert_dose), 5),
                                                 pert_dose))
...
compounds <- LFC %>%
  dplyr::distinct(pert_iname, pert_id, pert_dose, pert_idose, pert_plate, pert_time) %>%
...
```{r compound}
DT::datatable(compounds, options = list(dom = "t", ordering = FALSE),
              colnames = c("Compound name", "Compound ID", "Dose", "Dose with units", "Plate", "Time"),
              rownames = FALSE)
```
...
if (length(timepoints) > 1) {
  LFC_matrix %<>%
    reshape2::acast(ccle_name + culture ~ pert_dose + pert_time,
                    value.var = "LFC_cb", fun.aggregate = mean)
  LFC_matrix <- lapply(timepoints, function(x) LFC_matrix[, str_detect(colnames(LFC_matrix), x)])
  LFC_matrix <- do.call(cbind, LFC_matrix)
} else {
  LFC_matrix %<>%
    reshape2::acast(ccle_name + culture ~ pert_dose, 4,
                    value.var = "LFC_cb", fun.aggregate = mean)
}
...
missing <- lineages %>%
  dplyr::filter(!(abbreviation %in% lin_tab$abbreviation)) %>%
  dplyr::left_join(LIN_table,
                   by = c("lineage" = "feature"))

p_table <- lin_tab %>%
  distinct(lineage, pert_time, effect_size, q.value, pert_iname, pert_dose) %>%
...
if(nrow(p_table) > 0){
  p <- p_table %>%
    ...
    facet_wrap(pert_iname ~ pert_time, ncol = max(length(timepoints)/2, 1)) +
...
DT::datatable(lin_tab %>%
                dplyr::distinct(ccle_name, pert_time, lineage, abbreviation,
                                effect_size, q.value, pert_iname, pert_dose) %>%
                dplyr::group_by(abbreviation) %>%
                dplyr::mutate(n = n_distinct(ccle_name)) %>%
                dplyr::ungroup() %>%
                dplyr::distinct(pert_time, lineage, abbreviation, n,
                                effect_size, q.value, pert_iname, pert_dose) %>%
              ...
              colnames = c("Lineage", "Abbreviation", "Effect size", "q value", "Compound", "Time", "Dose", "# lines"))
```
...
if (!is.null(DRC)) {
  df <- DRC %>%
    ...
    dplyr::mutate(condition = ifelse("added_doses" %in% colnames(.),
                                     paste(varied_iname, added_compounds, added_doses),
                                     varied_iname))
  
  p <- df %>%
    ggplot(aes(x = auc, y = reorder(abbreviation, auc, function(x) -mean(x, na.rm = T)),
               color = abbreviation)) +
    geom_boxplot(outlier.alpha = 0) + geom_jitter() +
    facet_wrap(condition ~ pert_time, nrow = 1) + 
    theme(legend.position = "none") +
    labs(x = "AUC", y = "Lineage")
  p
} else if (n_doses == 1) {
  df <- LFC %>%
    dplyr::inner_join(lineages, by = "ccle_name")
  ...
  p <- df %>%
    ggplot(aes(x = LFC_cb, y = reorder(abbreviation, LFC_cb, function(x) -mean(x, na.rm = T)),
               color = abbreviation)) +
    geom_boxplot(outlier.alpha = 0) + geom_jitter() +
    facet_wrap(pert_dose ~ pert_time, nrow = 1) + 
    theme(legend.position = "none") +
    labs(x = "Log-fold change", y = "Lineage")
  p
} else {
...
if(nrow(p_table) > 0){
  p <- p_table %>%
    ...
    ylim(0, NA) +
    labs(x = "Effect Size", color = "Dose") +
    facet_wrap(pert_time ~ ., ncol = max(length(timepoints)/2, 1)) +
    geom_vline(aes(xintercept = 0), lty = 2, lwd =.5)
  plotly::ggplotly(p)
}

DT::datatable(MUT_table %>%
                dplyr::arrange(q.value, desc(abs(effect_size))) %>%
                dplyr::inner_join(mutations,
                                  by = c("feature" = "mutation")) %>%
                dplyr::group_by(feature) %>%
                dplyr::mutate(n = n_distinct(ccle_name)) %>%
                dplyr::ungroup() %>%
                dplyr::distinct(feature, pert_time, n,
                                effect_size, q.value, pert_iname, pert_dose) %>%
                dplyr::mutate(effect_size = signif(effect_size, 3),
                              q.value = signif(q.value, 3)),
              filter = "top", rownames = F, options = list(dom = "tp"),
              colnames = c("Mutation", "Effect size", "q value", "Compound", "Time", "Dose", "# lines")
)
```
...
if (!is.null(DRC)) {
  df <- DRC %>%
    dplyr::left_join(mutations, by = "ccle_name") %>%
    dplyr::group_by(across(any_of(c("mutation", "varied_iname", "added_compounds", "added_doses")))) %>%
    dplyr::mutate(n = n()) %>%
    dplyr::ungroup()
  
  top_muts <- MUT_table %>%
    dplyr::inner_join(df %>% dplyr::filter(n >= 5) %>% dplyr::distinct(mutation), by = c("feature" = "mutation")) %>%
    dplyr::filter(pert_dose == "log2.auc") %>%
    dplyr::arrange(q.value)
  
  top_muts <- c(top_muts$feature[1:20])
  
  df <- DRC %>%
    dplyr::left_join(mutations %>% dplyr::filter(mutation %in% top_muts), by = "ccle_name") %>%
    dplyr::mutate(condition = ifelse("added_doses" %in% colnames(.),
                                     paste(varied_iname, added_compounds, added_doses),
                                     varied_iname),
                  mutation = ifelse(is.na(mutation), "others", mutation))
  ...
  df <- LFC %>%
    dplyr::left_join(mutations %>% dplyr::filter(mutation %in% top_muts), by = "ccle_name") %>%
    dplyr::mutate(mutation = ifelse(is.na(mutation), "others", mutation))
...
for (feat in corr_feats) {
  ...
  dt <- DT::datatable(df %>%
                        dplyr::filter(q.val < 0.1 || rank <= 100) %>%
                        dplyr::select(feature, coef, q.val, pert_iname, pert_time, pert_dose) %>%
                        dplyr::arrange(q.val) %>%
                        dplyr::mutate(coef = signif(coef, 3),
                                      q.val = signif(q.val, 3)),
                      filter = "top", rownames = F, options = list(dom = "tp"),
                      colnames = c("Feature", "Correlation", "q value", "Compound", "Time", "Dose")
  )
...
DT::datatable(model_table %>%
                dplyr::select(pert_iname, pert_dose, model, pert_time, R2, PearsonScore) %>%
                dplyr::mutate_at(vars(R2, PearsonScore),
                                 funs(signif(., 3))) %>%
                dplyr::arrange(desc(R2)),
              filter = "top", rownames = F,
              colnames = c("Compound", "Dose", "Model", "Time", "R-squared", "Pearson Score"))
```
...
p <- df %>%
  dplyr::filter(rank == 1) %>%
  dplyr::select(feature, R2, PearsonScore, RF.imp.mean, pert_iname, pert_dose, pert_time) %>%
...
DT::datatable(df %>%
                dplyr::select(feature, R2, rank, RF.imp.mean, RF.imp.stability, RF.imp.sd, pert_iname, pert_dose, pert_time) %>%
                dplyr::mutate_at(vars(RF.imp.mean, RF.imp.sd, RF.imp.stability, R2),
                                 funs(signif(., 3))) %>%
                dplyr::arrange(rank),
              filter = "top", rownames = F, options = list(dom = "tp"),
              colnames = c("Feature", "r-squared", "Rank", "Feature importance", "Feature stability", "Feature importance SD",
                           "Compound", "Dose", "Time"))
```
