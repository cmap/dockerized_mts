...
## Synergy plots {.tabset .tabset-fade}
...
B <- apply(2^spread_lfc[,11:ncol(spread_lfc)], 1, prod)
...
### Viability versus Bliss expectation

```{r bliss scatter, fig.height=synergy_height}
spread_lfc$B <- pmin(B, 1)
spread_lfc$FC <- pmin(2^spread_lfc$LFC_cb, 1)

plots <- list()
for (i in 1:nrow(combos)) {
  bliss_plot <- highlight_key(spread_lfc %>% dplyr::inner_join(combos[i,]),
                                ~ccle_name, group = "Bliss scores")
  p <- plot_ly(bliss_plot) %>%
    add_trace(x = ~B, y = ~FC, type = "scatter", mode = "markers",
              hoverinfo = "text", text = ~ccle_name, opacity = 0.5) %>%
...
subplot(plots, nrows = max(1, ceiling(length(plots)/4)), shareX = F, shareY = F) %>%
  layout(title = pert_iname,
         annotations = list(
           list(x = 0, xshift = -50, y = 0.5,
                textangle = 270, text = "Combo fold-change",
                font = list(size = 14),
                showarrow = F, xref='paper', yref='paper',
                size = 48),
           list(x = 0.5, y = 0, yshift = -40,
                text = "Bliss expectation",
                font = list(size = 14),
                showarrow = F, xref='paper', yref='paper',
                size = 48)
         ))
...
for (i in 1:length(solo_comps)) {
  if (i < 1) {
    warning("No AUC data from combination any constituents alone, unable to generate combination report.")
    quit(save = "no")
  }
  comp <- solo_comps[[i]]
  df <- solo_drc %>%
...
subplot(plots, nrows = max(1, ceiling(length(plots)/3)), titleX = T, titleY = T, margin = 0.05)
```
