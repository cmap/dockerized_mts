...
spread_lfc %<>% dplyr::mutate(diff = abs(LFC_cb + dr))
spread_lfc %<>% dplyr::mutate(diff_exp = (LFC_cb_1 + LFC_cb_2 + dr))
#spread_lfc %<>% dplyr::mutate(flag = ifelse(diff < 1, 1, 0))

#spread_lfc %<>% dplyr::filter(diff > 1)
spread_lfc %>% ggplot() + geom_point(aes(x = diff_exp, y = diff))  + geom_abline()
...
spread_lfc %<>% dplyr::mutate(flag = ifelse((diff < 1 & S < 0), 1, 0))
