...
<center>
![Synergy Template](/Users/shiker/Desktop/synergy_template.png){width=50%}
</center>
...
The y axis represents the $q$ values of the synergy score, which are normalized to account for the variance of cell line abundance in DMSO and subsequently corrected to account for multiple hypothesis testing. The standard error of the measured LMFI values for each cell line and plate pair are assumed to be roughly the same across different perturbations and therefore can be estimated using the DMSO replicates. In interpreting the synergy scores, we recommend focusing more on effect size after providing a generous threshold on the q-values (i.e. 0.05 or 0.01)

**AUC plots**

The second tab (AUC plots) shows the AUC of the combination versus the Bliss expected AUC. The Bliss expected AUC assumes that independent effects are additive in log-scale or multiplicative in fold-change. Each value is calculated by taking the mean fold-change across doses with a given anchor dose as a proxy for AUC. This area can be approximated through Riemann-Sums, where the area of each dose response is estimated as a rectangle with an area of $\approx \frac{1}{d_2-d_1}FC_{d_1}$ where $d_2$ is the higher dose (upper bound) and $d_1$ is the lower dose (lower bound) and $FC_{d_1}$ is the fold change for $d_1$. Because upper dose is always three times higher than the lower dose, $d_2$ - $d_1$ = $d_{n+1}$ - $d_n$ = log(3) in log dose axis (x-axis of DRC), where n is the dose number. When the summation of these rectangles (across n doses) is taken, the approximate area under the DRC is:
