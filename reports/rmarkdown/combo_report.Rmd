...
spread_lfc <- combo_lfc %>%
  dplyr::inner_join(qc_table %>%
                      ...
                      dplyr::summarise(ctl_vehicle_mad = median(ctl_vehicle_mad), dr = median(dr), 
                                       .groups = "drop")) %>%  # QC info
  dplyr::distinct(pert_iname, pert_dose, ccle_name, culture, LFC_cb, ctl_vehicle_mad, dr) %>%
...
# computing actual difference between LFC of combination and dynamic range 
spread_lfc %<>% dplyr::mutate(dr_diff = abs(LFC_cb + dr))
spread_lfc %<>% dplyr::mutate(dr_diff_exp = (LFC_cb_1 + LFC_cb_2 + dr))


spread_lfc$S <- rowSums(spread_lfc[, c('LFC_cb_1', 'LFC_cb_2')]) - spread_lfc$LFC_cb # adding individual lfc and subtracting by combined lfc (S formula)

spread_lfc %<>% dplyr::mutate(flag = ifelse((dr_diff < 1 & S < 0), 1, 0)) # if LFC_cb values is within a range of 1 of dr and s < 0 assign flag value of 1 (could be noise)
...
for (i in 1:nrow(combos)) {
  synergy_plot <- highlight_key(spread_lfc %>% dplyr::filter(flag == 0) # filter for visualization only - not analysis 
 %>% dplyr::inner_join(combos[i,]),
...
for (i in 1:length(comps)) {
  ...
  for (j in 1:length(doses)) {
    ...
    p <- plot_ly(height = 200 * length(doses)) %>%
      ...
      layout(height = 600, xaxis = list(title = "Bliss expected AUC", range = c(0, 1)),
