...
LFC %<>% dplyr::arrange(as.numeric(pert_dose_sort))
...
corr_table %<>% 
  dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                ...
                pert_dose = ifelse(pert_dose %in% c("log2.auc", "log2.ic50"),
                                   paste(pert_iname, pert_dose, added_compounds, added_doses),
                                   pert_dose))
disc_table %<>% 
  dplyr::mutate(pert_dose = ifelse(!is.na(as.numeric(pert_dose)),
                                   signif(as.numeric(pert_dose), 5),
...
p_table <- lin_tab %>%
  distinct(lineage, effect_size, q.value, pert_iname, pert_dose, added_doses) %>%
...
```{r, fig.height=14}
if (!is.null(DRC) && length(DRC$varied_iname %>% unique()) == 1 && length(DRC$added_doses %>% unique) == 1) {
  df <- DRC %>%
