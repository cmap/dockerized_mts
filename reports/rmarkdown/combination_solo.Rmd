---
params:
  data_dir: '/Users/shiker/Documents/trouble_shoot/CPS008/PCPS017/O6-BENZYLGUANINE_TEMOZOLOMIDE'
...
rf_table <- data.table::fread(file.path(params$data_dir, "RF_table.csv")) %>%
  dplyr::left_join(model_table, by = c("model", "pert_iname", "pert_id", "pert_time", "pert_plate", "added_compounds", "added_ids", "added_doses", "pert_dose"))
...
DT::datatable(LFC %>%
              ...
              colnames = c("Cell line", "Culture", "Pool", "Dose",
                           "Vehicle", "log2-fold change", "Viability"), extensions = 'Buttons', options = list(dom = 'Blfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print'), lengthMenu = list(c(10,25,50,-1), c(10,25,50,"All"))))
...
DT::datatable(lin_tab %>%
                dplyr::inner_join(n_lines, by = c("lineage", "lin_abbreviation", "n_lines")) %>%
                dplyr::filter(n_lines >= 5) %>%
              ...
              filter = "top", rownames = F, extensions = 'Buttons', options = list(dom = 'Blfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print'), lengthMenu = list(c(10,25,50,-1), c(10,25,50,"All"))),
