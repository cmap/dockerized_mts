...
for (comp in varied_comps) {
  ...
  p <- plot_ly() %>%
    ...
    add_trace(data = curves_all,
              x = ~x, y = ~y, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.05, name = ~combo_title, split = ~as.factor(added_doses)) %>%
    add_trace(data = curves_bliss,
              x = ~x, y = ~bliss, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.05, name = ~bliss_title, split = ~as.factor(pert_dose)) %>%
    add_trace(data = curves_anchor,
              x = ~pert_dose, y = ~FC_cb, 
              type = "scatter", mode = "markers", opacity = 0.05, name = ~anchor_title, split = ~as.factor(pert_dose)) %>%
