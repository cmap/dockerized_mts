...
pseudo_auc %<>% dplyr::mutate(MSS = log2(B/FC)) %>% rename(varied_iname = pert_iname)
drc_meta %<>% dplyr::left_join(pseudo_auc %>% dplyr::select(cell_id, MSS, varied_iname) %>% dplyr::distinct(cell_id, MSS, varied_iname))


drc_meta %<>% dplyr::left_join(bliss_data_const %>% dplyr::select(cell_id, FC_cb, varied_iname) %>% dplyr::distinct(cell_id, FC_cb, varied_iname))
...
for (comp in varied_comps) {
  ...
  p <- plot_ly() %>%
    ...
    add_trace(data = curves_all,
              x = ~x, y = ~y, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.1, name = 'Combination', split = ~as.factor(added_doses)) %>%
    add_trace(data = curves_bliss,
              x = ~x, y = ~bliss, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.1, name = 'BLISS - No Interaction', split = ~as.factor(pert_dose)) %>%
