...
We fit a robust four-parameter logistic curve to the response of each cell line to treatment. The four parameters are upper limit, lower limit, slope, and EC50. The equation for the curve is defined:

$$f(x) = b + \frac{a-b}{1+e^{s\log{\frac{x}{EC50}}}}$$
This is implemented using R's dr4pl package, specifying an upper limit between 0.99-1.01 and a positive slope. 
...
for (comp in varied_comps) {
  ...
  curves_anchor <- highlight_key(bliss_data_const %>% dplyr::filter(is.finite(auc), varied_iname == comp) %>%
     dplyr::arrange(auc) %>% dplyr::select(ccle_name, pert_id, pert_dose, FC_cb, cell_id) %>% dplyr::distinct(ccle_name, pert_id, pert_dose, FC_cb, cell_id),
                            ~cell_id, group = comp)
  ...
  p <- plot_ly() %>%
    ...
    add_trace(data = curves_all,
              x = ~x, y = ~y, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.05, name = 'Combination', split = ~as.factor(added_doses)) %>%
    add_trace(data = curves_bliss,
              x = ~x, y = ~bliss, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.05, name = 'BLISS - No Interaction', split = ~as.factor(pert_dose)) %>%
    add_trace(data = curves_anchor,
              x = ~pert_dose, y = ~FC_cb, 
              type = "scatter", mode = "markers", opacity = 0.05, name = 'Anchor Dose Viability', split = ~as.factor(pert_dose)) %>%
  ...
  q <- plot_ly() %>%
    ...
    layout( legend = list(title = list(text = "MSS")),
      xaxis = list(title = "Bliss expected AUC", range = c(0, 1)),
            yaxis = list(title = "AUC", range = c(0, 1.05)), 
            hovermode = "closest",
