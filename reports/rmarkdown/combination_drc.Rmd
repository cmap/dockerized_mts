...
for (comp in varied_comps) {
  ...
  auc_path = file.path(plot_files_dir, 'auc_bliss_mss')
  dir.create(auc_path, showWarnings = FALSE)
  write.csv(export_pseudo_auc, file.path(auc_path, 'combo_auc_bliss_mss_table.csv'), row.names=FALSE)

  # save for reports landing page 
  write.csv(export_pseudo_auc, file.path(params$data_dir, 'combo_auc_bliss_mss_table.csv'), row.names=FALSE)
