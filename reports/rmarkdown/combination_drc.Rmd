...
drc_meta %<>% dplyr::left_join(bliss_data %>% dplyr::select(ccle_name, FC_cb) %>% dplyr::distinct(ccle_name, FC_cb))
...
for (comp in varied_comps) {
  ...
  cat(htmltools::knit_print.shiny.tag(dt))
    ...
    q <- plot_ly() %>%
    add_trace(data = compounds_all,
            x = ~mean_S, y = ~auc,
            type = "scatter", mode = "markers",
            hoverinfo = "text", text = ~ccle_name, opacity = 0.1, name = 'CombinationTreatment') 
