---
params:
  data_dir: '/Users/shiker/Documents/reports/CPS04_KRONOS/KB-00130742-012_KB-00282115-002'
  comp: 'KB-00130742-012_KB-00282115-002'
  data_dir_b: ''
  comp_b: ''
  data_dir_a: ''
  comp_a: ''
  qc_path: '/Users/shiker/Documents/reports/CPS04_KRONOS/data/CPS04_KRONOS_QC_TABLE.csv'
...
# const_a_path <-  file.path(plate_dir, const)
...
for (i in 1:nrow(conditions_const)) {
  ...
  d_const <- drc_const %>% dplyr::inner_join(condition_const)
...
bliss_data_const <- lfc_const %>% select(ccle_name, culture, LFC_cb) %>% dplyr::inner_join(drc_plots_const) %>% dplyr::mutate(FC_cb = 2^LFC_cb) %>% dplyr::mutate(bliss = FC_cb * y)
...
# DETERMINING ANCHOR DOSES
# pert_dose_1_list <- combo_lfc %>% splitstackshape::cSplit(splitCols = c("pert_iname", "pert_dose"),
#                           sep = "|", fixed = T,
#                           direction = "wide", drop = F, type.convert = T) %>% dplyr::select(pert_dose_1) 
# 
# const_1_doses <- pert_dose_1_list %>% dplyr::distinct() %>% dplyr::arrange(pert_dose_1)
# 
# const_1_anchors <- const_1_doses[which(table(pert_dose_1_list) == max(table(pert_dose_1_list), na.rm = TRUE))]$pert_dose_1
# 
# 
# pert_dose_2_list <- combo_lfc %>% splitstackshape::cSplit(splitCols = c("pert_iname", "pert_dose"),
#                           sep = "|", fixed = T,
#                           direction = "wide", drop = F, type.convert = T) %>% dplyr::select(pert_dose_2) 
# 
# const_2_doses <- pert_dose_2_list %>% dplyr::distinct() %>% dplyr::arrange(pert_dose_2)
# 
# const_2_anchors <- const_2_doses[which(table(pert_dose_2_list) == max(table(pert_dose_2_list), na.rm = TRUE))]$pert_dose_2
...
bliss_data_const_1 <- lfc_const %>% dplyr::filter(pert_iname == constituents[2]) %>% filter(round(pert_dose, 10) %in% anchor_2_doses) %>% dplyr::inner_join(drc_plots_const_1) %>% dplyr::mutate(FC_cb = 2^LFC_cb) %>% dplyr::mutate(bliss = FC_cb * y)

bliss_data_const_2 <- lfc_const %>% dplyr::filter(pert_iname == constituents[1]) %>% filter(round(pert_dose, 10) %in% anchor_1_doses) %>% dplyr::inner_join(drc_plots_const_2) %>% dplyr::mutate(FC_cb = 2^LFC_cb) %>% dplyr::mutate(bliss = FC_cb * y)
...
Dose response curves are fitted using a robust 4-parameter. The four parameters are upper limit, lower limit, slope, and EC50. The equation for the curve is defined:

$$f(x) = b + \frac{a-b}{1+e^{s\log{\frac{x}{EC50}}}}$$
This is implemented using the R's dr4pl package, specifying an upper limit between 0.99-1.01 and a positive slope. 
...
Combination - Dose response curve the compound being varied with an anchor compound and dose  <br> 
Bliss - The expected dose response curve assuming independent interaction of the single treatment compound and anchor compound. As such, the model assumes the log fold changes (LFCs) of the single compound and anchor compound for respective fixed doses are additive and their fold changes (FCs) are multiplicative. 

**How is AUC calculated for combination and expected Bliss?**
...
The mean synergy score (MSS) is an average metric of synergy across doses for a given compound. Because synergy score is in terms of LFC ($LFC_{a} + LFC_{b} - LFC_{c}$), the mean synergy score (MSS) is as follows:
...
drc_meta %<>% dplyr::left_join(pseudo_auc %>% dplyr::select(cell_id, MSS, varied_iname, added_doses) %>% dplyr::distinct(cell_id, MSS, varied_iname, added_doses))


drc_meta %<>% dplyr::left_join(bliss_data_const %>% dplyr::rename(added_doses = pert_dose) %>% dplyr::select(cell_id, FC_cb, varied_iname, added_doses) %>% dplyr::distinct(cell_id, FC_cb, varied_iname, added_doses))
...
for (comp in varied_comps) {
  ...
  # anchor_FC_mark <- bliss_data %>% dplyr::filter(x == min(bliss_data$x))
  # 
  # curves_anchor <- highlight_key(anchor_FC_mark %>%
  #                             dplyr::filter(is.finite(auc), varied_iname == comp) %>%
  #                             dplyr::arrange(auc),
  #                           ~cell_id, group = comp)
  ...
  # annot <- list(x = anchor_FC_mark$x-1,
  #                 y = anchor_FC_mark$bliss,
  #                 text = 'ADV',
  #                 xref = "x",
  #                 yref = "y",
  #                 showarrow = TRUE,
  ...
  p <- plot_ly() %>%
    add_trace(data = curves_all_a,
            ...
            hoverinfo = "text", text = ~ccle_name, opacity = 0.01, name = 'Single Treatment') %>%
    
    add_trace(data = curves_all,
              x = ~x, y = ~y, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.01, name = 'Combination', split = ~as.factor(added_doses)) %>%
    add_trace(data = curves_bliss,
              x = ~x, y = ~bliss, 
              type = "scatter", mode = "lines", line = list(shape = "line"),
              hoverinfo = "text", text = ~ccle_name, opacity = 0.01, name = 'BLISS - No Interaction', split = ~as.factor(pert_dose)) %>%
    # add_trace(data = curves_anchor,
    #           x = ~x, y = ~bliss, 
    #           type = "scatter", mode = "markers", opacity = 0.1, name = 'Anchor Dose Viability' ) %>%
  ...
  q <- plot_ly() %>%
    add_trace(data = pseudo_auc_comp,
            ...
            hoverinfo = "text", text = ~ccle_name, name = 'CombinationTreatment', marker=list(colorbar=list(text = 'Mean Syngery Score'))) %>%
    config(doubleClick = "reset") %>%
    layout( legend = list(title = list(text = "MSS")),
      xaxis = list(title = "Bliss expected AUC", range = c(0, 1)),
