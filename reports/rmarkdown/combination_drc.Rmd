...
This report displays the dose response curves (DRC) fit for `r params$comp`. The next tab shows DRC for single treatment compounds (compound with varied dose), combination treatments (compound with varied dose and compound with anchor dose), and BLISS expected model. Below this graph is an interactive table where you may click a row on the table to highligh a particular cancer cell line of interest. 
...
MSS - Mean synergy score across doses (higher scores suggest synergy and lower scores suggest antagonism) 

**What is synergy score? How is the MSS calculated?**


The means synergy score (MSS) is an average metric of synergy across doses for a given compound. Synergy score is in terms of LFC ($LFC_{a} + LFC_{b} - LFC_{c}), therefore the mean synergy score (MSS) is as follows:
