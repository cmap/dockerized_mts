...
$$MSS = log(\frac{AUC_{combo}}{AUC_{bliss}}) \approx log(\frac{ \frac{1}{n}\sum_{i=1}^nFC_{c_i}}{\frac{1}{n}\sum_{i=1}^nFC_{a_i}FC_b})  $$ 
...
drc_meta %<>% dplyr::left_join(bliss_data %>% dplyr::select(cell_id, FC_cb, varied_iname) %>% dplyr::distinct(cell_id, FC_cb, varied_iname))
...
for (comp in varied_comps) {
  ...
  q <- plot_ly() %>%
    add_trace(data = pseudo_auc_comp,
            ...
            hoverinfo = "text", text = ~ccle_name, opacity = 0.1, opacity = 0.1, name = 'CombinationTreatment', marker=list(colorbar=list(text = 'Mean Syngery Score'))) %>%
