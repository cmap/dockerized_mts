...
qc_table %>%
  dplyr::group_by(ccle_name, culture, pool_id, pert_plate, prism_replicate) %>%
  dplyr::summarise(pass = sum(pass) > n()/2, .groups = "drop") %>%
...
for (i in 1:length(plates)) {
  ...
  for (j in 1:ncol(combos)) {
    ...
    replicates_cor[[j]] <- cor(df1[overlap, ], df2[overlap, ]) %>% diag()
...
This plot shows the proportion of perturbations killing a given cell line on a given plate to 30% viability and the 90th percentile of LFC for that cell line. Each point is a cell line. Cell lines in the table are killed in either 0 or more that 40% of perturbations on a plate.
...
DT::datatable(cl_summary %>% 
                dplyr::filter(prop_killed == 0 | prop_killed > 0.4) %>%
